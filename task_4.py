...
def build_tree_from_heap(heap, i):
    if i < len(heap):
        root = Node(heap[i])
        root.left = build_tree_from_heap(heap, 2 * i + 1)
        root.right = build_tree_from_heap(heap, 2 * i + 2)
        return root
    else:
        return None
...
if __name__ == "__main__":
    nodes = [77, 78, 90, 6, 15, 8, 2]
    heapq.heapify(nodes)
    root_tree = build_tree_from_heap(nodes, 0)
    draw_tree(root_tree)
